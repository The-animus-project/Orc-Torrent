...
export default function MobileApp() {
  ...
  const finishBootstrap = useCallback(async (value: AndroidBootstrap) => {
    ...
    configureDaemonApi({ baseUrl: value.baseUrl });
    await waitForDaemon();
    ...
    async (file?: PickedTorrentFile, magnetUri?: string) => {
      ...
      try {
        if (bootstrap?.killSwitchEnabled && !bootstrap.vpnActive) {
          throw new Error("Connect your VPN before adding a torrent while VPN transfer pause is enabled");
        }
        const added = file
  ...
  const remove = async (torrent: Torrent, deleteData: boolean) => {
    const wording = deleteData
      ? "remove this torrent and permanently delete its downloaded files"
      : "remove this torrent but keep its files";
  ...
  if (!bootstrap) {
    return (
      <main className="mobile-shell mobile-centered">
        <div className="orc-spinner" />
        <p>Starting ORC…</p>
  ...
  if (!bootstrap.storageReady) {
    return (
      <main className="mobile-shell onboarding">
        ...
        <p className="lead">
          Choose an ORC subfolder in shared storage. Android keeps the files there even if the app is removed.
        </p>
        <div className="onboarding-note">
          <strong>Choose a subfolder</strong>
          <span>
            Android does not allow apps to use the top-level Downloads folder. Create or select something like
            Downloads/ORC.
          </span>
        </div>
        <label className="switch-row">
          <span>
            <strong>Pause transfers when VPN disconnects</strong>
            <small>Pause all peer traffic when the VPN disconnects</small>
          </span>
          <input
            type="checkbox"
            checked={onboardingKillSwitch}
            onChange={(event) => setOnboardingKillSwitch(event.target.checked)}
          />
        </label>
        <button className="primary-button" onClick={() => void chooseStorage()} disabled={busy}>
          {busy ? "Opening…" : "Choose download folder"}
        </button>
        {error && <p className="mobile-error">{error}</p>}
      </main>
    );
  ...
  return (
    <main className="mobile-shell">
      <header className="mobile-header">
        <div>
          <p className="eyebrow">ORC TORRENT</p>
          <h1>{selected ? selected.name : tab === "downloads" ? "Downloads" : tab[0].toUpperCase() + tab.slice(1)}</h1>
        </div>
        {selected ? (
          <button className="icon-button" aria-label="Back" onClick={() => setSelected(null)}>
            ←
          </button>
        ) : (
          <div
            className={`connection-dot ${bootstrap.vpnActive ? "protected" : ""}`}
            title={bootstrap.vpnActive ? "VPN connected" : "VPN not connected"}
          />
        )}
      </header>
      ...
      {error && (
        <button className="mobile-error error-banner" onClick={() => setError(null)}>
          {error}
        </button>
      )}
      ...
      <section className="mobile-content">
        {selected ? (
          <TorrentDetail
            ...
            onAction={act}
            onRemove={remove}
            onPriority={async (file, priority) => {
              await patchJson(`/torrents/${selected.id}/file-priority`, { path: file.path, priority });
              setContent((current) =>
                current
                  ? {
                      files: current.files.map((item) =>
                        item.path.join("/") === file.path.join("/") ? { ...item, priority } : item
                      ),
                    }
                  : current
              );
            }}
            onOpen={(index) => bridge.openDownloadedFile(selected.id, index)}
            onShare={(index) => bridge.shareDownloadedFile(selected.id, index)}
          />
        ) : tab === "downloads" ? (
          <>
            <div className="summary-card">
              <span>{torrents.length} torrents</span>
              <strong>↓ {speed(totals)}</strong>
            </div>
            <div className="torrent-list">
              {torrents.map((torrent) => (
                <TorrentCard
                  key={torrent.id}
                  torrent={torrent}
                  status={statuses[torrent.id]}
                  onOpen={() => {
                    history.pushState({ torrent: torrent.id }, "");
                    setSelected(torrent);
                  }}
                  onAction={act}
                />
              ...
              {!torrents.length && (
                <div className="empty-state">
                  ...
                  <p>Add a magnet link or a .torrent file to begin.</p>
                  <button onClick={() => openTab("add")}>Add torrent</button>
                </div>
              )}
            </div>
          </>
          ...
          <div className="panel-stack">
            <section className="mobile-panel">
              ...
              <button
                ...
                onClick={() => void addTorrent(undefined, magnet)}
            ...
            <button
              ...
              onClick={() => void bridge.pickTorrentFile().then((file) => file && addTorrent(file))}
            >
              <strong>Choose .torrent file</strong>
              <span>Open from Files, Drive, or another app</span>
            </button>
          </div>
        ) : tab === "privacy" ? (
          <div className="panel-stack">
            <section className="privacy-hero">
              <span className={bootstrap.vpnActive ? "shield active" : "shield"}>◆</span>
              <h2>{bootstrap.vpnActive ? "VPN protected" : "VPN not detected"}</h2>
              <p>
                {bootstrap.killSwitchEnabled
                  ? "VPN transfer pause is armed. ORC closes its sockets if this VPN disappears."
                  : "Transfers use the active Android network."}
              </p>
            </section>
            <label className="switch-row mobile-panel">
              <span>
                <strong>Pause transfers when VPN disconnects</strong>
                <small>Requires a connected Android VPN before transfers can run</small>
              </span>
              <input
                type="checkbox"
                checked={bootstrap.killSwitchEnabled}
                onChange={(event) => void updatePolicy({ killSwitchEnabled: event.target.checked })}
              />
            </label>
            <button
              className="secondary-button"
              onClick={() => void bridge.bootstrap().then((value) => value && setBootstrap(value))}
            >
              Refresh VPN status
            </button>
          </div>
          ...
          <div className="panel-stack">
            <section className="mobile-panel">
              ...
              <label className="switch-row">
                <span>
                  <strong>Allow cellular data</strong>
                  <small>Off by default. Active work is rescheduled when changed.</small>
                </span>
                <input
                  type="checkbox"
                  checked={bootstrap.allowCellular}
                  onChange={(event) => void updatePolicy({ allowCellular: event.target.checked })}
                />
              </label>
            </section>
            <section className="mobile-panel">
              ...
              <label className="switch-row">
                <span>
                  <strong>Stop at ratio</strong>
                  <small>Default is 1.0, on Wi-Fi</small>
                </span>
                <input
                  type="checkbox"
                  checked={seeding.ratio_limit_enabled}
                  onChange={(event) => void updateSeeding({ ...seeding, ratio_limit_enabled: event.target.checked })}
                />
              </label>
              <label className="number-row">
                ...
                <input
                  ...
                  min="0"
                  step="0.1"
                  value={seeding.ratio_limit}
                  onChange={(event) =>
                    void updateSeeding({ ...seeding, ratio_limit: Math.max(0, Number(event.target.value)) })
                  }
                />
              </label>
              <label className="switch-row">
                <span>
                  ...
                  <small>Optionally stop after a number of minutes</small>
                </span>
                <input
                  type="checkbox"
                  checked={seeding.seed_time_limit_enabled}
                  onChange={(event) =>
                    void updateSeeding({ ...seeding, seed_time_limit_enabled: event.target.checked })
                  }
                />
              </label>
              {seeding.seed_time_limit_enabled && (
                <label className="number-row">
                  <span>Minutes</span>
                  <input
                    type="number"
                    min="1"
                    value={seeding.seed_time_minutes}
                    onChange={(event) =>
                      void updateSeeding({ ...seeding, seed_time_minutes: Math.max(1, Number(event.target.value)) })
                    }
                  />
                </label>
              )}
            ...
            <section className="mobile-panel">
              <h2>Storage</h2>
              <p>{bootstrap.storageLabel || "ORC folder"}</p>
              <button className="secondary-button" onClick={() => void chooseStorage()}>
                Reconnect or change folder
              </button>
              <small>
                Changing folders requires an empty torrent queue. Reselect the same folder to restore access.
              </small>
            </section>
            <section className="mobile-panel about">
              <h2>About</h2>
              <p>ORC Torrent for Android</p>
              <a href="https://github.com/The-animus-project/Orc-Torrent/releases" target="_blank" rel="noreferrer">
                GitHub releases ↗
              </a>
            </section>
          </div>
...
function TorrentCard({
  ...
  onOpen(): void;
  onAction(torrent: Torrent, action: "start" | "stop"): void;
}) {
    ...
    <article className="torrent-card">
      <button className="card-main" onClick={onOpen}>
        ...
        <div className="card-meta">
          <span className={`state ${status?.state ?? "stopped"}`}>{status?.state ?? "stopped"}</span>
          <span>
            ↓ {speed(status?.down_rate_bps)} · ↑ {speed(status?.up_rate_bps)}
          </span>
        </div>
      </button>
      <button
        className="round-action"
        aria-label={torrent.running ? "Pause" : "Resume"}
        onClick={() => void onAction(torrent, torrent.running ? "stop" : "start")}
      >
        {torrent.running ? "Ⅱ" : "▶"}
      </button>
    </article>
...
function TorrentDetail({
  ...
  torrent: Torrent;
  status: TorrentStatus | null | undefined;
  content: Content | null;
  busy: boolean;
  onAction(torrent: Torrent, action: "start" | "stop"): void;
  onRemove(torrent: Torrent, deleteData: boolean): void;
  onPriority(file: ContentFile, priority: ContentFile["priority"]): Promise<void>;
  onOpen(index: number): Promise<boolean>;
  onShare(index: number): Promise<boolean>;
}) {
  const progress = Math.max(0, Math.min(100, (status?.progress ?? 0) * 100));
  return (
    <div className="panel-stack detail">
      <section className="detail-hero">
        ...
        <div>
          <span className={`state ${status?.state ?? "stopped"}`}>{status?.state ?? "stopped"}</span>
          <p>
            {bytes(status?.downloaded_bytes)} of {bytes(status?.total_bytes)}
          </p>
          <p>
            ↓ {speed(status?.down_rate_bps)} · ↑ {speed(status?.up_rate_bps)}
          </p>
          <p>
            Ratio {(status?.ratio ?? 0).toFixed(2)} · {status?.peers_seen ?? 0} peers
          </p>
        </div>
      </section>
      <div className="detail-actions">
        <button
          ...
          onClick={() => void onAction(torrent, torrent.running ? "stop" : "start")}
        >
          {torrent.running ? "Pause" : "Resume"}
        </button>
        <button className="secondary-button" onClick={() => void onRemove(torrent, false)}>
          Remove
        </button>
      </div>
      <section className="mobile-panel">
        ...
        {content?.files.map((file, index) => (
          <div className="file-row" key={`${index}-${file.path.join("/")}`}>
            <div>
              <strong>{file.path.at(-1)}</strong>
              <small>
                {bytes(file.size)} · {file.downloaded ? "Complete" : file.priority}
              </small>
            </div>
            <select
              aria-label={`Priority for ${file.path.at(-1)}`}
              value={file.priority}
              onChange={(event) => void onPriority(file, event.target.value as ContentFile["priority"])}
            >
            ...
            {file.downloaded && (
              <div className="file-actions">
                <button onClick={() => void onOpen(index)}>Open</button>
                <button onClick={() => void onShare(index)}>Share</button>
              </div>
            )}
          </div>
        ))}
      ...
      <section className="danger-zone">
        <button onClick={() => void onRemove(torrent, true)}>Remove and delete data</button>
      </section>
    </div>
  );
}

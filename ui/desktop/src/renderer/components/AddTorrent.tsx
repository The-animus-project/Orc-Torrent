...
export const AddTorrent = memo<AddTorrentProps>(
  ({ online, wallet, torrents, onTorrentAdded, onSelectTorrent, onExistingTorrentFound, onError, onSuccess }) => {
    ...
    const addMagnet = useCallback(async () => {
      ...
      try {
        ...
        if (m.startsWith("http://") || m.startsWith("https://")) {
          ...
          try {
            ...
            if (b64.length > MAX_BASE64_SIZE) {
              throw new Error(
                `Encoded torrent file too large (${(b64.length / 1024 / 1024).toFixed(2)}MB). Maximum is 10MB.`
              );
            }
            ...
            const res = await addTorrentB64ToDaemon(b64, savePath, undefined, m);
            if (!res?.id) {
        ...
        const res = await addMagnetToDaemon(m, savePath);
        if (!res?.id) {
          ...
          try {
            ...
            if (fileSizeMB > 1) {
              logger.logWithPrefix(
                "AddTorrent",
                `Processing large torrent file (${fileSizeMB.toFixed(2)}MB), this may take a moment...`
              );
            }
            ...
            logger.logWithPrefix(
              "AddTorrent",
              `Uploading torrent file: ${file.name} (${(b64.length / 1024 / 1024).toFixed(2)}MB base64 encoded)...`
            );
            ...
            if (!res?.id) {
              logger.errorWithPrefix(
                "AddTorrent",
                `Daemon rejected torrent file add request for ${file.name} (no ID returned)`
              );
              onError("Daemon rejected torrent add request");
              return;
            }
            ...
            if (errorMsg.includes("timed out") || errorMsg.includes("timeout")) {
              onError(
                `Upload timed out. The torrent file may be too large or the daemon is slow to respond. Try a smaller file or check daemon logs.`
              );
            } else {
              onError(errorMsg);
            }
        ...
        <div className="stack">
          <div className="fieldRow">
            ...
            >
              {loadingUrl ? (
                <>
                  <SpinnerInline /> DOWNLOADING...
                </>
              ) : loadingMagnet ? (
                <>
                  <SpinnerInline /> ADDING...
                </>
              ) : (
                "ADD"
              )}
          ...
          <div className="fieldRow" style={{ alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            <span className="kLabel" style={{ marginRight: 4 }}>
              Save to:
            </span>
            <span
              className="kValue"
            ...
            <button
              ...
              disabled={!online}
              aria-label="Choose folder for torrent"
            >
              Choose folder
            </button>
            {savePath && (

...
export const NetworkPosture = memo<NetworkPostureProps>(
  ({ netPosture, netifs, online, selectedTorrent, onUpdate, onError, onSuccess }) => {
        ...
        <div className="stack">
          ...
          {isPrivateMode && (
            <div className={`npWarn ${isProtected ? "ok" : "bad"}`}>
              PRIVATE MODE AUDIT:{" "}
              {isProtected
                ? "Leak protection and bind interface configured (advisory)"
                : "Enable leak protection and bind to a network interface to reduce exposure risk"}

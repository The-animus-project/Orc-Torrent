...
interface FileSelectionDialogProps {
  ...
  enforceMediaPolicy?: boolean;
}

export const FileSelectionDialog = memo<FileSelectionDialogProps>(
  ({ isOpen, onClose, torrentId, torrentName, torrentSize, onConfirm, onError, enforceMediaPolicy = false }) => {
    ...
    const buildFileTree = useCallback((entries: FileEntry[]): FileNode[] => {
      ...
      entries.forEach((entry, index) => {
        ...
        const isSelected = entry.priority !== "skip" && (!enforceMediaPolicy || isDownloadAllowedForPath(fullPath));
                ...
                const addNewFiles = (nodes: FileNode[]) => {
                  nodes.forEach((node) => {
                    ...
                    if (!merged.has(node.path)) {
                      const allowed = !enforceMediaPolicy || isDownloadAllowedForPath(node.path);
                      const isSelected = node.size > 0 ? allowed && node.selected : true;
    ...
    const toggleFileSelection = useCallback(
      (path: string, selected: boolean) => {
        if (enforceMediaPolicy && selected && !isDownloadAllowedForPath(path)) {
          onError(`AnimUS blocked "${path.split("/").pop() ?? path}": ${downloadBlockReason(path)}`);
          return;
        }
        setFileSelections((prev) => {
      ...
      [torrentId, enforceMediaPolicy, onError]
    );
      ...
      (select: boolean) => {
        setFileSelections((prev) => {
          ...
          const updateNode = (node: FileNode) => {
            if (node.size > 0) {
              // Only toggle files, not directories
    ...
    const handleConfirm = useCallback(async () => {
      ...
      try {
        ...
        const collectSelected = (nodes: FileNode[]) => {
          nodes.forEach((node) => {
            if (node.size > 0) {
              // Only include files
              const isSelected = fileSelections.get(node.path) ?? true;
              if (isSelected && (!enforceMediaPolicy || isDownloadAllowedForPath(node.path))) {
        ...
        if (files.length > 0) {
          ...
          const skipSuccessCount = skipResults.filter((r) => r.status === "fulfilled" && r.value.success).length;
          ...
          const downloadSuccessCount = downloadResults.filter(
    ...
    const renderFileNode = useCallback(
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "4px", width: "20px" }}>
                ...
                {isFile && (
                  <input
                    type="checkbox"
                    checked={isSelected}
                    disabled={enforceMediaPolicy && !isDownloadAllowedForPath(node.path)}
                    onChange={(e) => {
                      e.stopPropagation();
      ...
      [expandedPaths, toggleExpand, toggleFileSelection, enforceMediaPolicy, fileSelections]
    );
    ...
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="Add Torrent">
          ...
          {loading ? (
            ...
            <div style={{ padding: "20px", textAlign: "center", color: "var(--text-muted)" }}>
              ...
              <div style={{ fontSize: "12px", marginBottom: "16px", color: "var(--text-muted)" }}>
                {torrentId
                  ? "For magnet links, files will appear after metadata is fetched from peers. This may take a few seconds."
                  : "Please wait for the torrent to be added."}
              </div>
              {torrentId && (
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", alignItems: "center" }}>
                  ...
                  <div
                    style={{ fontSize: "11px", color: "var(--text-muted)", fontStyle: "italic", textAlign: "center" }}
                  >
            ...
            <>
              ...
              >
                <div>
                  <button className="btn small" onClick={() => toggleAll(!allSelected)} style={{ marginRight: "8px" }}>
                    {allSelected ? "SELECT NONE" : "SELECT ALL"}
                  </button>
                </div>
                <div style={{ fontSize: "12px", color: "var(--text-muted)" }}>
                  Selected: <span style={{ fontWeight: 600, color: "var(--text)" }}>{fmtBytes(selectedSize)}</span> of{" "}
                  {fmtBytes(totalSize)}
                </div>
              </div>
              <div
              ...
              >
                ...
                <div style={{ padding: "4px 0" }}>{files.map((file) => renderFileNode(file))}</div>
              </div>
            </>
          )}
        ...
        >
          <button className="btn" onClick={onClose} style={{ minWidth: "80px" }}>
            Cancel
          </button>
          <button className="btn primary" onClick={handleConfirm} disabled={loading} style={{ minWidth: "100px" }}>

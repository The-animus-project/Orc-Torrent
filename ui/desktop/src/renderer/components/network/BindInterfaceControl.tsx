...
export const BindInterfaceControl = memo<BindInterfaceControlProps>(
  ({ netPosture, netifs, vpnStatus, online, onUpdate, onError, onSuccess }) => {
            ...
            >
              ...
              {netifs.map((n) => (
                <option key={n} value={n} style={n === vpnStatus?.interfaceName ? { fontWeight: "bold" } : {}}>
                  {n}
                  {n === vpnStatus?.interfaceName ? " (VPN)" : ""}
                </option>
              ))}
            </select>
            {vpnStatus?.detected && vpnStatus.interfaceName && (
              <button className="btn ghost small" onClick={handleLockToVpn} disabled={!online}>
                LOCK TO VPN
              </button>
            )}
          </div>
          <button className="btn" onClick={handleApply} disabled={!online || loading}>

...
export const TorrentDetails = memo<TorrentDetailsProps>(
  ({ torrent, status, overlay, online, onUpdate, onError, onSuccess }) => {
    ...
    const setProfile = useCallback(
      async (mode: TorrentMode, hops?: number) => {
        if (!torrent || loadingAction) return;
            ...
            <div className="card">
              ...
              {!status ? (
                ...
                <>
                  ...
                  <div
                    style={{ marginTop: "8px", marginBottom: "12px", fontSize: "0.9em", color: "var(--text-muted)" }}
                  >
                    {fmtSizeProgress(status.downloaded_bytes, status.total_bytes)}
                    {status.total_bytes > 0 && <span style={{ marginLeft: "8px" }}>({fmtPct(progress)})</span>}
                  </div>

...
export const TorrentRowSignal = memo<TorrentRowSignalProps>(
  ({
    ...
    // Single poll for both pieces and heartbeat data.
    // 2s is fast enough for visual feedback and avoids flooding the daemon with
    // N * high-frequency requests (the old 150ms heartbeat poll was ~6.7 req/s PER row).
    useEffect(() => {
      if (pollIntervalMs <= 0) return;
      fetchSnapshot();
      const id = setInterval(fetchSnapshot, pollIntervalMs);
    ...
    useEffect(() => {
      ...
      const binsChanged =
        ...
        prevBinsRef.current.some(
          (bin, i) => bins[i] && (bin.have_ratio !== bins[i].have_ratio || bin.min_avail !== bins[i].min_avail)
        );

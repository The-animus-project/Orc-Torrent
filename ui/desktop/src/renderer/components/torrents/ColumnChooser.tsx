...
export const ColumnChooser = memo<ColumnChooserProps>(
  ({ visibleColumns, onColumnsChange, savedLayouts, onSaveLayout, onLoadLayout }) => {
    ...
    if (!isOpen) {
      return (
        <button className="btn ghost" onClick={() => setIsOpen(true)} title="Choose Columns">
          COLUMNS
        </button>
      );
    }
          ...
          <div className="columnChooserSection">
            ...
            <div className="columnChooserLayouts">
              {savedLayouts.map((layout) => (
                <button key={layout} className="btn ghost small" onClick={() => onLoadLayout(layout)}>
                  {layout}
                </button>
              ))}
            </div>
            <div className="columnChooserSave">
              ...
              />
              <button className="btn small" onClick={handleSaveLayout} disabled={!layoutName.trim()}>
                Save
              </button>
            </div>
          </div>

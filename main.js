...
function getDesktopProjectRoot() {
  const candidates = [
  ...
  for (const candidate of candidates) {
    if (
      ...
      existsSync(path.join(candidate, "splash.html"))
    ) {
      return candidate;
    }
  }
...
function resolveBuiltArtifactPath(...segments) {
  const candidates = [
...
function getDefaultNotificationSoundsDir() {
  const candidates = [
...
function persistAppThemeMode(source) {
  try {
    writeFileSync(
      getAppThemeMetaPath(),
...
function pushThemeStateToWindow(win, state) {
  if (
    !win ||
    win.isDestroyed() ||
    !win.webContents ||
...
function pushThemeStateToSplash(state) {
  if (
    !splashWindow ||
  ...
  splashWindow.webContents
    .executeJavaScript(
      `window.__applyOrcTheme?.(${JSON.stringify(state)});`,
      true,
    )
    .catch(() => {
...
function broadcastAppThemeState(state = getCurrentAppThemeState()) {
...
function getSplashFallbackDataUrl(theme) {
  const background = theme === "dark" ? "#0f1115" : "#f8f3ea";
  const foreground = theme === "dark" ? "#ffffff" : "#1b1a17";
  const muted =
    theme === "dark" ? "rgba(255,255,255,0.5)" : "rgba(27,26,23,0.5)";
  const border =
    theme === "dark" ? "rgba(255,255,255,0.2)" : "rgba(31,29,26,0.15)";
  const borderTop =
    theme === "dark" ? "rgba(255,255,255,0.9)" : "rgba(31,29,26,0.85)";
  return (
      ...
      if (process.platform === "win32") {
        if (errorCode === "EACCES" || errorCode === "EPERM") {
          daemonLogStream.write(
            `[WINDOWS] Permission error - may need Administrator rights\n`,
          );
        } else if (errorCode === "ENOENT") {
          daemonLogStream.write(
            `[WINDOWS] File not found - check antivirus hasn't blocked files\n`,
          );
        } else if (errorCode === "EADDRINUSE") {
          daemonLogStream.write(
            `[WINDOWS] Port in use - another instance may be running\n`,
          );
        }
      }
    } catch (logErr) {
  ...
  splashWindow
    ...
    .catch((err) => {
      ...
      if (splashWindow && !splashWindow.isDestroyed()) {
        splashWindow
          .loadURL(getSplashFallbackDataUrl(themeState.resolved))
          .catch((fallbackErr) => {
...
async function revealMainWindowWhenReady() {
  ...
  if (!(await isDaemonHealthy())) {
    console.warn(
      "Daemon not healthy when renderer became ready, attempting to start...",
    );
    void startDaemonIfNeeded().catch((err) => {
      console.error(
        "Background daemon start failed while revealing main window:",
        err,
      );
    });
  }
...
function createWindow() {
  ...
  mainWindow.webContents.on(
    ...
    (event, errorCode, errorDescription, validatedURL, isMainFrame) => {
      ...
      console.error(
        `[Main] Error code: ${errorCode}, Description: ${errorDescription}`,
      );
  ...
  mainWindow.webContents.once("did-finish-load", () => {
    setTimeout(() => {
      if (!mainWindowRevealed && mainWindowReadyToShow) {
        console.warn(
          "[Main] Renderer ready signal timed out, revealing window after did-finish-load fallback.",
        );
...
function handleMagnetLink(magnetUrl) {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    try {
      ...
      console.log(
        `[Magnet] Sent magnet link to renderer: ${trimmed.substring(0, 50)}...`,
      );
    } catch (err) {
    ...
    console.log(
      `[Magnet] Storing magnet link for later (window not ready): ${trimmed.substring(0, 50)}...`,
    );
    pendingMagnetLinks.push(trimmed);
  }
}
function extractMagnetFromArgv(argv) {
  ...
  try {
    ...
    if (!existsSync(dirPath)) {
      try {
        ...
        const errCode = mkdirErr.code;
        if (errCode === "EACCES" || errCode === "EPERM") {
          return {
            writable: false,
            error: `Permission denied creating directory: ${dirPath}`,
          };
        }
        return {
          writable: false,
          error: `Failed to create directory: ${mkdirErr}`,
        };
      }
    }
    ...
    try {
      ...
      const errCode = accessErr.code;
      if (errCode === "EACCES" || errCode === "EPERM") {
        return {
          writable: false,
          error: `Permission denied accessing directory: ${dirPath}`,
        };
      }
      return {
        writable: false,
        error: `Cannot access directory: ${accessErr}`,
      };
    }
    ...
    try {
      ...
      if (errCode === "EACCES" || errCode === "EPERM") {
        return {
          writable: false,
          error: `Permission denied writing to directory: ${dirPath}`,
        };
      }
      return {
        writable: false,
        error: `Cannot write to directory: ${writeErr}`,
      };
    }
  } catch (err) {
    return {
      writable: false,
      error: `Error checking directory permissions: ${err}`,
    };
  }
}
/**
 ...
 */
function checkBinaryPermissions(binPath) {
  try {
    ...
    try {
      ...
      if (errCode === "EACCES" || errCode === "EPERM") {
        return {
          accessible: false,
          error: `Permission denied accessing binary: ${binPath}`,
        };
      }
    ...
    return {
      accessible: false,
      error: `Error checking binary permissions: ${err}`,
    };
  }
}
/**
 ...
 */
async function findWritableLogDirectory() {
  const candidates = [];
  ...
  for (const candidate of candidates) {
    ...
    if (check.writable) {
      console.log(
        `[Log Directory] Using writable directory: ${candidate.path} (${candidate.name})`,
      );
      return { path: path.join(candidate.path, "daemon.log") };
    } else {
      console.warn(
        `[Log Directory] Directory not writable: ${candidate.path} (${candidate.name}) - ${check.error}`,
      );
    }
  }
  ...
  console.warn(
    `[Log Directory] All candidates failed, using last resort: ${lastResort}`,
  );
  return {
    path: lastResort,
    error: "Could not verify any log directory is writable",
  };
}
...
function httpRequestJson(method, pathname, headers) {
...
async function waitForHealthy(timeoutMs = DAEMON_HEALTH_CHECK_TIMEOUT_MS) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (await isDaemonHealthy()) {
      ...
      console.log(
        `[HEALTH CHECK] Daemon became healthy after ${elapsed}ms (${attempt} attempts)`,
      );
  ...
  console.warn(
    `[HEALTH CHECK] Daemon did not become healthy within ${timeoutMs}ms (${attempt} attempts)`,
  );
  return false;
}
async function waitForReady(timeoutMs = DAEMON_HEALTH_CHECK_TIMEOUT_MS * 2) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (await isDaemonReady()) {
      const elapsed = Date.now() - start;
      console.log(
        `[READINESS CHECK] Daemon became ready after ${elapsed}ms (${attempt} attempts)`,
      );
      return true;
    }
    await new Promise((r) => setTimeout(r, delay));
  ...
  console.warn(
    `[READINESS CHECK] Daemon did not become ready within ${timeoutMs}ms (${attempt} attempts)`,
  );
  return false;
}
...
async function findProcessUsingPort(port) {
  ...
  try {
    ...
    const isOrcDaemon = await new Promise((resolve) => {
      const chunks = [];
      ...
      child.on("close", () => {
        const lowerOutput = Buffer.concat(chunks)
          .toString("utf8")
          .toLowerCase();
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      ...
      if (
        ...
        console.warn(
          `[Port Hygiene] Permission denied killing process ${pid}. May need Administrator rights.`,
        );
        resolve(false);
      } else if (
        ...
        console.error(
          `[Port Hygiene] Failed to kill process ${pid} (exit ${code}):`,
          stderr.trim(),
        );
        resolve(false);
      }
...
async function cleanupStalePeerPorts() {
  if (process.platform !== "win32") {
  ...
  for (const port of PEER_PORTS) {
    ...
    if (pid && !killedPids.has(pid)) {
      ...
      if (!isHealthy) {
        console.log(
          `[Port Hygiene] Found stale orc-daemon (PID: ${pid}) using peer port ${port}`,
        );
        ...
        console.log(
          `[Port Hygiene] Port ${port} in use by healthy daemon (PID: ${pid}), skipping cleanup`,
        );
      }
    }
  }
  if (cleaned > 0) {
    ...
    console.log(
      `[Port Hygiene] Cleaned up ${cleaned} stale process(es) from peer ports`,
    );
  } else {
...
async function cleanupStaleProcessesOnPort(options) {
  ...
  try {
    ...
    if (!portInUse) {
      console.log(
        `[Port Hygiene] Port ${DAEMON_PORT} is not in use, no cleanup needed`,
      );
      return { cleaned: false };
    }
    ...
    if (isHealthy && !forceKill) {
      console.log(
        `[Port Hygiene] Port ${DAEMON_PORT} is in use by a healthy daemon, no cleanup needed`,
      );
      return { cleaned: false };
    }
    if (isHealthy && forceKill) {
      console.log(
        `[Port Hygiene] Replacing external daemon with app-owned daemon (ORC_DOWNLOAD_DIR will be set)`,
      );
    }
    // Port is in use - find and kill the process (stale or external we're replacing)
    console.warn(
      `[Port Hygiene] Port ${DAEMON_PORT} is in use but daemon is not healthy - attempting cleanup`,
    );
    const pid = await findProcessUsingPort(DAEMON_PORT);
    if (!pid) {
      console.warn(
        `[Port Hygiene] Could not find orc-daemon process using port ${DAEMON_PORT}`,
      );
      return { cleaned: false };
    }
    console.log(
      `[Port Hygiene] Found stale orc-daemon (PID: ${pid}) using port ${DAEMON_PORT}`,
    );
    const killed = await killProcess(pid);
    if (killed) {
      ...
      if (stillInUse) {
        console.warn(
          `[Port Hygiene] Port ${DAEMON_PORT} is still in use after killing process ${pid}`,
        );
        return { cleaned: false, pid };
      }
      console.log(
        `[Port Hygiene] Port ${DAEMON_PORT} is now free after cleanup`,
      );
...
function resolveDaemonBinary() {
  ...
  console.log(
    `[Binary Resolution] isDev: ${isDev}, app.isPackaged: ${app.isPackaged}`,
  );
  console.log(
    `[Binary Resolution] process.resourcesPath: ${process.resourcesPath || "undefined"}`,
  );
  ...
  if (packaged) {
    console.log(
      `[Binary Resolution] Packaged path candidate: ${packaged} (exists: ${existsSync(packaged)})`,
    );
  }
  ...
  const candidates = [
    ...
    {
      path: packaged,
      name: "packaged (resources/bin)",
      check: !isDev && packaged,
    },
    ...
    {
      path: packaged,
      name: "packaged (resources/bin) fallback",
      check: isDev && packaged,
    },
    {
      path: packagedNextToExe,
      name: "packaged (exe/bin) fallback",
      check: isDev,
    },
    {
      path: packagedSameDir,
      name: "packaged (exe dir) fallback",
      check: isDev,
    },
        ...
        if (existsSync(candidate.path)) {
          ...
          if (process.platform === "win32") {
            try {
              ...
              console.warn(
                `Binary exists at ${candidate.path} but may not be accessible:`,
                accessErr,
              );
          ...
          console.log(
            `[Binary Resolution] Found binary at: ${candidate.path} (${candidate.name})`,
          );
          return candidate.path;
        }
      } catch (err) {
        console.warn(
          `Error checking ${candidate.name} binary at ${candidate.path}:`,
          err,
        );
      }
    }
  ...
  console.warn(
    `No binary found in expected locations, using fallback: ${fallback}`,
  );
  ...
  for (const candidate of candidates) {
    if (candidate.path) {
      ...
      console.warn(
        `  ${candidate.name}: ${candidate.path} (exists: ${exists})`,
      );
    }
  }
...
async function startDaemonIfNeeded() {
  ...
  if (isRestarting) {
    console.warn(
      "[START] Already restarting, skipping duplicate start attempt",
    );
    return false;
  }
  ...
  console.log(
    `[Port Hygiene] Checking for stale processes on port ${DAEMON_PORT}...`,
  );
  const cleanupResult = await cleanupStaleProcessesOnPort();
  if (cleanupResult.cleaned) {
    console.log(
      `[Port Hygiene] Cleaned up stale process (PID: ${cleanupResult.pid})`,
    );
    if (daemonLogStream) {
      try {
        daemonLogStream.write(
    ...
    console.warn(
      `[Port Hygiene] Could not clean up process ${cleanupResult.pid} - startup may fail`,
    );
  }
  // Also clean up peer ports (6881-6890) to prevent DirectTransport binding failures
  console.log(
    `[Port Hygiene] Checking for stale processes on peer ports (6881-6890)...`,
  );
  ...
  if (!logDirCheck.writable) {
    console.warn(
      `[Pre-flight] Log directory may not be writable: ${logDirCheck.error}`,
    );
  ...
  try {
    ...
    daemonLogStream = fallbackLog; // Type workaround, but will work for writing
    log = fallbackLog;
    fallbackLog.write(
      `[WARNING] Could not create log file at ${logFile}, logging to stderr\n`,
    );
    fallbackLog.write(
  ...
  try {
    ...
    try {
      ...
      if (process.platform === "win32") {
        log.write(
          `[ERROR] On Windows, ensure the binary exists and is not blocked by antivirus\n`,
        );
      }
      if (log !== process.stderr && typeof log.end === "function") {
        log.end();
      }
    } catch (logErr) {
...
- Ensure the file is not read-only`;
    ...
    try {
      ...
      if (process.platform === "win32") {
        if (errorMsg.includes("ENOENT")) {
          log.write(
            `[WARNING] Binary not found - check if antivirus blocked it\n`,
          );
          log.write(`[WARNING] Binary path was: ${bin}\n`);
        } else if (errorMsg.includes("EACCES")) {
          log.write(
            `[WARNING] Permission denied - may need to run as Administrator\n`,
          );
        } else if (errorMsg.includes("spawn")) {
          log.write(
            `[WARNING] Failed to spawn process - check binary permissions and antivirus\n`,
          );
        }
      }
    } catch (logErr) {
      ...
      lines.forEach((line) => {
        if (line.trim()) {
          ...
          if (
            ...
            line.includes("daemon listening")
          ) {
            console.log(
              `[STARTUP] Daemon is listening - should become healthy soon`,
            );
          }
        }
      });
  ...
  daemonProc.on("exit", (code, signal) => {
    ...
    try {
      ...
      console.error(
        `[Daemon Exit] Process exited with code: ${code}, signal: ${signal || "none"}`,
      );
      ...
      if (process.platform === "win32" && code !== null && code !== 0) {
        ...
        if (code === 4) {
          ...
          log.write(
            `[ERROR] Check with: netstat -ano | findstr :${DAEMON_PORT}\n`,
          );
          log.write(
            `[INFO] Port hygiene will attempt cleanup before restart\n`,
          );
        }
      }
    } catch (logErr) {
    ...
    if (daemonRestartAttempts >= MAX_RESTART_ATTEMPTS) {
      try {
        log.write(
          `[ERROR] Maximum restart attempts (${MAX_RESTART_ATTEMPTS}) reached. Stopping automatic restarts.\n`,
        );
    ...
    daemonRestartTimeout = setTimeout(async () => {
      ...
      if (isShuttingDown || !mainWindow || mainWindow.isDestroyed()) {
        console.log(
          "Skipping daemon restart - app is shutting down or window closed",
        );
        isRestarting = false;
        return;
      }
      ...
      if (isRestarting) {
        console.warn(
          "[RESTART] Already restarting, skipping duplicate restart attempt",
        );
        return;
      }
      ...
      if (timeSinceLastRestart < MIN_RESTART_COOLDOWN_MS) {
        const remainingCooldown =
          MIN_RESTART_COOLDOWN_MS - timeSinceLastRestart;
        console.warn(
      ...
      try {
        console.log(
          `Attempting to restart daemon (attempt ${daemonRestartAttempts}/${MAX_RESTART_ATTEMPTS})...`,
        );
        ...
        if (healthy) {
          ...
          setTimeout(async () => {
            if (!isShuttingDown && (await isDaemonHealthy())) {
              ...
              try {
                if (daemonLogStream && daemonLogStream !== process.stderr) {
                  daemonLogStream.write(
                    `[INFO] Daemon stable, restart counter reset to 0\n`,
                  );
                }
              } catch (logErr) {
                // Ignore log errors
          ...
          console.warn(
            "Daemon restarted but did not become healthy within timeout",
          );
  ...
  console.log(
    `Waiting for daemon to become healthy (timeout: ${DAEMON_HEALTH_CHECK_TIMEOUT_MS / 1000}s)...`,
  );
  try {
    log.write(
    ...
    log.write(
      `[HEALTH CHECK] Checking if daemon is listening on ${DAEMON_HOST}:${DAEMON_PORT}...\n`,
    );
  } catch (logErr) {
    // Ignore log errors
  }
  ...
  if (!up) {
    ...
    try {
      ...
      if (daemonProc) {
        try {
          ...
          if (process.platform === "win32") {
            ...
            const exitCode = daemonProc.exitCode;
            if (exitCode !== null && exitCode !== undefined) {
              log.write(
                `[DIAGNOSTIC] Process has exited with code: ${exitCode}\n`,
              );
            } else {
              log.write(
                `[DIAGNOSTIC] Process appears to be running (PID: ${daemonProc.pid})\n`,
              );
            }
          } else {
            ...
            try {
              if (daemonProc.pid) {
                ...
                log.write(
                  `[DIAGNOSTIC] Process is running (PID: ${daemonProc.pid})\n`,
                );
              }
            } catch (killErr) {
              log.write(
                `[DIAGNOSTIC] Process may have exited (kill check failed)\n`,
              );
            }
          }
        } catch (procErr) {
          log.write(
            `[DIAGNOSTIC] Could not check process status: ${procErr}\n`,
          );
        }
      } else {
      ...
      log.write(
        `[DIAGNOSTIC] Checking if port ${DAEMON_PORT} is accessible...\n`,
      );
      ...
      try {
        const testSocket = net.createConnection(
          ...
          () => {
            log.write(
              `[DIAGNOSTIC] Port ${DAEMON_PORT} is accessible (connection successful)\n`,
            );
            testSocket.destroy();
          },
        );
        testSocket.on("error", (err) => {
          log.write(
            `[DIAGNOSTIC] Port ${DAEMON_PORT} connection failed: ${err.message}\n`,
          );
          if (err.message.includes("ECONNREFUSED")) {
            log.write(
              `[DIAGNOSTIC] Port is not listening - daemon may not have started properly\n`,
            );
...
Linux/Mac: lsof -i :${DAEMON_PORT}`;
    ...
    console.log(
      `Daemon is healthy and reachable on ${DAEMON_HOST}:${DAEMON_PORT}`,
    );
    // Wait for daemon to be ready (fully initialized) before allowing operations
    console.log(
      `Waiting for daemon to become ready (timeout: ${(DAEMON_HEALTH_CHECK_TIMEOUT_MS * 2) / 1000}s)...`,
    );
    try {
      log.write(
        `[READINESS CHECK] Daemon is healthy, waiting for readiness...\n`,
      );
    } catch (logErr) {
      // Ignore log errors
    }
    ...
    if (!ready) {
      console.warn(
        `Daemon is healthy but not ready within timeout - operations may fail`,
      );
      try {
        log.write(
          `[WARNING] Daemon health check passed but readiness check timed out\n`,
        );
      } catch (logErr) {
        // Ignore log errors
      }
...
async function gracefulShutdownIfOwned() {
  ...
  while (Date.now() - start < 3000) {
    ...
    if (procRef.exitCode !== null) break;
  ...
  if (
    logStreamRef &&
    logStreamRef !== process.stderr &&
    typeof logStreamRef.end === "function"
  ) {
    try {
      logStreamRef.end();
    } catch (err) {
...
async function checkWindowsFirewallRule(ruleName) {
  return new Promise((resolve) => {
    ...
    exec(command, { encoding: "utf8" }, (error, stdout, stderr) => {
      ...
      const ruleExists =
        stdout.includes(ruleName) && !stdout.includes("No rules match");
      const isManaged =
        stdout.includes("Group Policy") || stdout.includes("GP");
 ...
 */
async function checkWindowsFirewallOutboundRules() {
  return new Promise((resolve) => {
    ...
    exec(command, { encoding: "utf8" }, (error, stdout, stderr) => {
      ...
      const hasOutboundRules =
        stdout.trim().length > 0 && stdout.toLowerCase().includes("outbound");
      resolve({ exists: hasOutboundRules });
    });
  });
}
async function checkIfFirewallManaged() {
  return new Promise((resolve) => {
...
async function addWindowsFirewallRulesBatch(
  rules,
  exePath,
  baseRuleName = "ORC TORRENT BitTorrent Peer",
) {
  return new Promise((resolve) => {
    const escapeForCmd = (str) => str.replace(/"/g, '""');
    ...
    for (const rule of rules) {
      ...
      for (const prof of profilesToUse) {
        for (const proto of protocolsToUse) {
          const ruleSuffix =
            profilesToUse.length > 1 || protocolsToUse.length > 1
              ? ` (${prof}/${proto})`
              : "";
          ...
          const createNetshCommand = (direction) => {
    ...
    try {
      ...
      exec(
        ...
        (error, stdout, stderr) => {
          ...
          if (error) {
            ...
            const batchMatch = stdout.match(
              /BATCH_RESULT:\s*Added=(\d+)\s+Skipped=(\d+)/,
            );
            if (batchMatch) {
      ...
      resolve({
        success: false,
        error: `Failed to create temporary script file: ${writeError}`,
        added: 0,
        skipped: 0,
      });
    }
...
async function addWindowsFirewallRule(options) {
  return new Promise((resolve) => {
    const {
      ruleName,
      exePath,
    ...
    const protocolsToUse = protocol === "both" ? ["tcp", "udp"] : [protocol];
    const createRule = (prof, proto, portSuffix = "") => {
      const ruleSuffix =
        profilesToUse.length > 1 || protocolsToUse.length > 1 || portSuffix
          ? ` (${prof}/${proto}${portSuffix})`
          : "";
      ...
      const createRuleForDirection = (direction) => {
        const directionSuffix = direction === "out" ? " Out" : "";
        const fullRuleNameWithDir = `${ruleName}${directionSuffix}${ruleSuffix}`;
        const escapedRuleNameForNetsh = escapeForCmd(
          fullRuleNameWithDir,
        ).replace(/'/g, "''");
      ...
      const createPsScript = (netshArgs) => {
        const psScript = [
...
async function removeWindowsFirewallRule(ruleName) {
  return new Promise((resolve) => {
            ...
            if (error) {
              ...
              if (
                ...
                exec(
                  elevatedCommand,
                  { encoding: "utf8" },
                  (elevError, elevStdout, elevStderr) => {
                    if (elevError) {
                      ...
                      if (
                        !elevErrorMsg.includes("not found") &&
                        !elevErrorMsg.includes("cannot find")
                      ) {
                        resolve({
                          success: false,
                          error:
                            elevError.message || elevStderr || "Unknown error",
                        });
                        return;
                      }
                    }
  ...
  app.on("second-instance", (event, commandLine) => {
    ...
    for (const arg of commandLine) {
      if (arg && arg.toLowerCase().endsWith(".torrent")) {
        console.log(
          `[Second Instance] Found torrent file in command line: ${arg}`,
        );
        ...
        if (existsSync(normalizedPath)) {
          ...
          console.warn(
            `[Second Instance] Torrent file not found: ${normalizedPath}`,
          );
 ...
 */
function normalizeTorrentFilePath(filePath) {
  if (!filePath) {
    return filePath;
  }
  ...
  try {
    ...
    console.warn(
      `[Path Normalization] path.resolve failed, using original path: ${resolveErr}`,
    );
    normalized = normalized.replace(/\//g, path.sep);
  }
  return normalized;
}
function checkTorrentFileAssociation() {
  if (process.platform !== "win32") {
    return {
      isRegistered: true,
      details: "File association check only available on Windows",
    };
  }
  if (!app.isPackaged) {
    return {
      isRegistered: false,
      details: "File associations only work in packaged/installed builds",
    };
  }
  try {
    ...
    const isInstalled =
      execPath.includes("Program Files") || execPath.includes("AppData");
    if (isInstalled) {
      return {
        isRegistered: true,
...
function handleTorrentFile(filePath) {
  if (!filePath || !filePath.toLowerCase().endsWith(".torrent")) {
    console.warn(
      `[Torrent File] Invalid file path or not a .torrent file: ${filePath}`,
    );
    return;
  }
  try {
    ...
    console.log(
      `[Torrent File] Successfully read file: ${fileName} (${(fileBuffer.length / 1024).toFixed(2)}KB)`,
    );
        ...
        try {
          ...
          if (mainWindow.webContents && !mainWindow.webContents.isDestroyed()) {
            ...
            if (url && url !== "about:blank") {
              mainWindow.webContents.send("open-torrent-file", data);
              console.log(
                `[Torrent File] Sent torrent file to renderer: ${fileName}`,
              );
              return true;
            } else {
              console.log(
                `[Torrent File] WebContents not ready yet (URL: ${url}), will retry...`,
              );
            }
          }
        } catch (err) {
          console.error(
            "[Torrent File] Failed to send torrent file to renderer:",
            err,
          );
        }
      }
    ...
    if (!sendToRenderer()) {
      ...
      torrentFileRetryInterval = setInterval(() => {
        ...
        if (sendToRenderer()) {
          ...
          console.log(
            `[Torrent File] Successfully sent torrent file after ${retryCount} retries`,
          );
          ...
          console.error(
            `[Torrent File] Failed to send torrent file after ${maxRetries} retries`,
          );
          // Show error to user if window is available
          if (mainWindow && !mainWindow.isDestroyed()) {
            dialog.showErrorBox(
  ...
  .then(async () => {
    try {
      ...
      protocol.handle("app", (request) => {
        ...
        if (
          url === NOTIFICATION_SOUND_CUSTOM_URL ||
          url.startsWith(`${NOTIFICATION_SOUND_CUSTOM_URL}?`)
        ) {
          ...
          try {
            ...
            const soundPath = path.join(
              userData,
              NOTIFICATION_SOUND_BASENAME + ext,
            );
        ...
        if (url.startsWith(NOTIFICATION_SOUND_DEFAULT_URL_PREFIX)) {
          try {
            const filename = decodeURIComponent(
              url
                .slice(NOTIFICATION_SOUND_DEFAULT_URL_PREFIX.length)
                .replace(/\?.*$/, ""),
            );
            ...
            if (
              !existsSync(soundPath) ||
              path.extname(soundPath).toLowerCase() !== ".mp3"
            ) {
              return new Response("", { status: 404 });
            }
            const buf = readFileSync(soundPath);
      ...
      ipcMain.handle("app-theme:get", async () => getCurrentAppThemeState());
      ipcMain.handle("app-theme:set", async (_event, source) => {
      ...
      const VALID_PROFILES = ["private", "public", "domain", "all"];
      const MIN_PORT = 1;
      ...
      ipcMain.handle("firewall:check", async () => {
        ...
        try {
          const inbound = await checkWindowsFirewallRule(
            FIREWALL_BASE_RULE_NAME,
          );
          ...
          return {
            ...
            error: inbound.error,
            checkAccessDenied:
              inbound.checkAccessDenied ?? outbound.checkAccessDenied,
          };
        } catch (err) {
      ...
      ipcMain.handle("firewall:check-managed", async () => {
        if (process.platform !== "win32") return false;
        return checkIfFirewallManaged();
      });
      ipcMain.handle("firewall:add-rule", async (_event, options) => {
        if (process.platform !== "win32") {
          return {
            success: false,
        ...
        return addWindowsFirewallRulesBatch(
          [{ port, protocol, profile }],
          exePath,
          FIREWALL_BASE_RULE_NAME,
        );
      });
      ipcMain.handle("firewall:add-rules-batch", async (_event, options) => {
        if (process.platform !== "win32") {
          return {
            success: false,
            error: "Firewall management is only available on Windows",
        ...
        if (ports.length > MAX_PORTS_BATCH) {
          return {
            success: false,
            error: `Maximum ${MAX_PORTS_BATCH} ports per batch`,
          };
        }
        ...
        if (validPorts.length !== ports.length) {
          return {
            success: false,
            error: `All ports must be integers between ${MIN_PORT} and ${MAX_PORT}`,
          };
        }
      ...
      ipcMain.handle("dialog:choose-save-folder", async () => {
      ...
      ipcMain.handle("notification-sound:choose", async () => {
        const win = BrowserWindow.getFocusedWindow() ?? mainWindow;
      ...
      ipcMain.handle(
        "notification-sound:set-default",
        async (_event, filename) => {
          ...
          try {
            writeFileSync(
              metaPath,
              JSON.stringify({ type: "default", filename: base }),
            );
            return true;
          } catch (err) {
      ...
      ipcMain.handle("notification-sound:get-defaults", async () => {
        try {
          const dir = getDefaultNotificationSoundsDir();
          ...
          const mp3Names = files
            .filter(
              (f) =>
                f.isFile() && path.extname(f.name).toLowerCase() === ".mp3",
            )
            .map((f) => f.name);
      ...
      ipcMain.handle("notification-sound:get-preference", async () => {
        const userData = app.getPath("userData");
        const metaPath = path.join(userData, NOTIFICATION_SOUND_META);
      ...
      ipcMain.handle("notification-sound:get-url", async () => {
        const userData = app.getPath("userData");
        const metaPath = path.join(userData, NOTIFICATION_SOUND_META);
        ...
        try {
          ...
          if (meta.type === "default" && meta.filename) {
            ...
            return existsSync(soundPath)
              ? buildDefaultNotificationSoundUrl(meta.filename)
              : null;
          }
          const ext = meta.ext ?? ".wav";
          const soundPath = path.join(
      ...
      ipcMain.handle("notification-sound:clear", async () => {
        const userData = app.getPath("userData");
        const metaPath = path.join(userData, NOTIFICATION_SOUND_META);
      ...
      ipcMain.handle("daemon:log-path", async () => {
        return currentDaemonLogPath;
      });
      ipcMain.handle("daemon:open-log", async () => {
      ...
      ipcMain.handle("daemon:cleanup-port", async () => {
        try {
      ...
      ipcMain.handle("daemon:start", async () => {
        try {
          if (isRestarting) {
            return {
              success: false,
              error: "Daemon is already starting or restarting",
            };
          }
      ...
      ipcMain.handle("daemon:stop", async () => {
        try {
          if (!daemonSpawnedByApp) {
            return {
              success: false,
              error: "Daemon was not started by this application",
            };
          }
      ...
      ipcMain.handle("daemon:read-logs", async (_event, lines = 100) => {
        try {
      ...
      ipcMain.handle("daemon:watch-logs", (event) => {
        try {
          if (!currentDaemonLogPath) {
            return { success: false, error: "Log file path not available" };
          }
      ...
      if (process.platform !== "darwin") {
        try {
          ...
          if (!isDefault) {
            ...
            if (registered) {
              console.log(
                "[Protocol] Successfully registered as default magnet protocol handler",
              );
            } else {
              console.warn(
            ...
            console.log(
              "[Protocol] Already registered as default magnet protocol handler",
            );
          }
        } catch (err) {
          console.error(
            "[Protocol] Error during magnet protocol registration:",
            err,
          );
      ...
      if (process.platform === "win32") {
        ...
        console.log(
          `[File Association] Status: ${associationStatus.isRegistered ? "Registered" : "Not Registered"}`,
        );
      ...
      console.log(
        `[Startup] Starting daemon (isDev: ${isDev}, isPackaged: ${app.isPackaged})...`,
      );
      console.log(
        `[Startup] process.resourcesPath: ${process.resourcesPath || "undefined"}`,
      );
      ...
      if (!(await isDaemonHealthy())) {
        console.warn(
          "Daemon not healthy when window ready, attempting to start...",
        );
        await startDaemonIfNeeded();
      }
        ...
        if (mainWindow && !mainWindow.isDestroyed()) {
          ...
          if (isHealthy) {
            ...
            if (consecutiveHealthFailures > 0) {
              console.log(
                `[HEALTH CHECK] Daemon recovered after ${consecutiveHealthFailures} failures`,
              );
            ...
            if (daemonSpawnedByApp && daemonProc) {
              try {
                if (process.platform === "win32") {
                  ...
                  if (exitCode !== null && exitCode !== undefined) {
                    console.warn(
                      `[HEALTH CHECK] Daemon process has exited (code: ${exitCode})`,
                    );
                    processDead = true;
                  } else {
                    ...
                    if (daemonProc.pid) {
                      try {
                        const result = execSync(
                          `tasklist /FI "PID eq ${daemonProc.pid}" /NH`,
                          {
                            encoding: "utf8",
                            timeout: 2000,
                        ...
                        if (
                          !resultLower.includes("orc-daemon") &&
                          !resultLower.includes(pidStr)
                        ) {
                          console.warn(
                        ...
                        if (
                          ...
                          console.warn(
                            `[HEALTH CHECK] Could not verify process existence via tasklist: ${errMsg}`,
                          );
                        }
                      }
                    }
                  ...
                  try {
                    if (daemonProc.pid) {
                      ...
                      console.warn(
                        `[HEALTH CHECK] Process PID is null, treating as dead`,
                      );
                      processDead = true;
                    }
                  } catch (killErr) {
                    console.warn(
                      `[HEALTH CHECK] Daemon process appears to be dead: ${killErr}`,
                    );
                    processDead = true;
                  }
                }
                ...
                console.warn(
                  `[HEALTH CHECK] Error checking process state: ${err}`,
                );
              ...
              console.warn(
                `[HEALTH CHECK] Process reference is null but we spawned it - process is dead`,
              );
              processDead = true;
            }
            ...
            if (
              processDead ||
              consecutiveHealthFailures >= MAX_CONSECUTIVE_FAILURES
            ) {
              if (processDead) {
                console.warn(
                  `[HEALTH CHECK] Daemon process is dead, forcing restart...`,
                );
              ...
              if (
                ...
                try {
                  ...
                  if (restarted) {
                    ...
                    setTimeout(async () => {
                      ...
                      if (healthy) {
                        console.log(
                          `[HEALTH CHECK] Daemon restarted successfully`,
                        );
                      } else {
                        console.warn(
                          `[HEALTH CHECK] Daemon restart may have failed`,
                        );
                      }
                    }, 2000);
                  }
                  ...
                  console.error(
                    `[HEALTH CHECK] Error during restart attempt:`,
                    restartErr,
                  );
                ...
                console.log(
                  `[HEALTH CHECK] Restart already in progress, skipping duplicate restart`,
                );
              }
            }
          }
      ...
      if (process.platform !== "darwin") {
        ...
        console.log(
          `[Startup] Checking command line arguments for torrent files...`,
        );
        ...
        for (const arg of process.argv) {
          if (arg && arg.toLowerCase().endsWith(".torrent")) {
            ...
            if (existsSync(normalizedPath)) {
              console.log(
                `[Startup] Torrent file exists, will open after window is ready`,
              );
              ...
              console.warn(
                `[Startup] Torrent file not found: ${normalizedPath}`,
              );

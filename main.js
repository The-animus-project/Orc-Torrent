...
import { NOTIFICATION_SOUND_CUSTOM_URL, NOTIFICATION_SOUND_DEFAULT_URL_PREFIX, NOTIFICATION_SOUNDS_DIR_NAME, buildDefaultNotificationSoundUrl, isSafeNotificationSoundFilename, notificationSoundMetaToPreference, sortNotificationSoundFilenames, } from "../shared/notificationSoundRegistry.js";
import { fromElectronThemeSource, isAppThemeMode, toElectronThemeSource, } from "../shared/appThemeRegistry.js";
...
function getIconPath() {
    ...
    if (isDev) {
        ...
        if (preferIco) {
            if (existsSync(devIco))
                return devIco;
            if (existsSync(devFlatIco))
                return devFlatIco;
            if (existsSync(devPng))
                return devPng;
        }
        else {
            if (existsSync(devPng))
        ...
        if (preferIco) {
            if (existsSync(orcIco))
                return orcIco;
            if (existsSync(orcPng))
                return orcPng;
        }
        else {
            if (existsSync(orcPng))
    ...
    if (process.resourcesPath) {
        ...
        if (preferIco) {
            if (existsSync(resourcesIco))
                return resourcesIco;
            if (existsSync(resourcesAppIco))
                return resourcesAppIco;
            if (existsSync(packagedIco))
                return packagedIco;
            if (existsSync(packagedFlatIco))
                return packagedFlatIco;
            if (existsSync(packagedPng))
                return packagedPng;
        }
        else {
            if (existsSync(packagedPng))
...
function createWindow() {
    ...
    mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL, isMainFrame) => {
        ...
        if (isMainFrame) {
            ...
            dialog.showErrorBox("Failed to Load UI", `Failed to load the application UI:\n\n${errorDescription}\n\nURL: ${validatedURL}\nError Code: ${errorCode}`);
        }
    });
        ...
        mainWindow.loadFile(indexHtml).catch((err) => {
            ...
            if (mainWindow && !mainWindow.isDestroyed()) {
                ...
                mainWindow.loadFile(fallbackPath).catch((fallbackErr) => {
                    ...
                    dialog.showErrorBox("Failed to Load UI", `Failed to load the UI HTML file.\n\nError: ${err.message}\n\nApp Path: ${appPath}\n__dirname: ${__dirname}\n\nPlease rebuild the application.`);
                });
            }
            else {
                dialog.showErrorBox("Failed to Load UI", `Failed to load the UI HTML file.\n\nError: ${err.message}\n\nApp Path: ${appPath}\n__dirname: ${__dirname}\n\nPlease rebuild the application.`);
...
function httpRequestJson(method, pathname, headers) {
    return new Promise((resolve, reject) => {
        const req = http.request({ host: DAEMON_HOST, port: DAEMON_PORT, path: pathname, method, timeout: DAEMON_HTTP_TIMEOUT_MS, headers }, (res) => {
            let data = "";
...
function httpGetJson(pathname) {
    return new Promise((resolve, reject) => {
        const req = http.request({ host: DAEMON_HOST, port: DAEMON_PORT, path: pathname, method: "GET", timeout: DAEMON_HTTP_TIMEOUT_MS }, (res) => {
            let data = "";
            res.on("data", (c) => (data += c));
            res.on("end", () => {
                try {
...
async function findProcessUsingPort(port) {
    ...
    try {
        ...
        const isOrcDaemon = await new Promise((resolve) => {
            ...
            child.on("close", () => {
                const lowerOutput = Buffer.concat(chunks).toString("utf8").toLowerCase();
    ...
    return new Promise((resolve) => {
        ...
        child.on("close", (code) => {
            ...
            if (stderr.includes("Access is denied") ||
                ...
                console.error(`[Port Hygiene] Failed to kill process ${pid} (exit ${code}):`, stderr.trim());
                resolve(false);
            }
        });
    });
    ...
    if (!binCheck.accessible) {
        ...
        if (altBin !== bin) {
            ...
            if (altCheck.accessible) {
                ...
                dialog.showErrorBox("Permission Error", `Cannot access daemon binary.\n\n${binCheck.error}\n\nPlease check:\n- File permissions\n- Antivirus software\n- Try running as Administrator`);
                return false;
            }
        }
        else {
...
- Ensure the file is not read-only`;
    ...
    daemonProc.on("exit", (code, signal) => {
        ...
        if (daemonRestartAttempts >= MAX_RESTART_ATTEMPTS) {
            ...
            if (mainWindow && !mainWindow.isDestroyed()) {
                dialog.showErrorBox("Daemon Crashed", `The daemon has crashed ${MAX_RESTART_ATTEMPTS} times and automatic restart has been disabled.\n\nPlease restart the application.\n\nExit code: ${code}\nLog: ${logFile}`);
            }
            return;
        }
...
async function gracefulShutdownIfOwned() {
    ...
    if (logStreamRef && logStreamRef !== process.stderr && typeof logStreamRef.end === "function") {
        try {
            logStreamRef.end();
        }
        catch (err) {
...
const RUN_AS_ADMIN_MSG = "Administrator rights are required. Right-click ORC TORRENT → Run as administrator, then try the firewall fix again.";
function isElevationError(msg) {
    const m = msg.toLowerCase();
    return (m.includes("elevation") ||
...
async function addWindowsFirewallRulesBatch(rules, exePath, baseRuleName = "ORC TORRENT BitTorrent Peer") {
    return new Promise((resolve) => {
        ...
        for (const arg of commandLine) {
            if (arg && arg.toLowerCase().endsWith(".torrent")) {
                ...
                if (existsSync(normalizedPath)) {
                    ...
                    if (mainWindow && !mainWindow.isDestroyed()) {
                        dialog.showErrorBox("Torrent File Not Found", `The torrent file could not be found:\n\n${normalizedPath}\n\nPlease verify the file exists and try again.`);
                    }
                }
            }
        }
...
app.whenReady().then(async () => {
    try {
        ...
        ipcMain.handle("firewall:add-rules-batch", async (_event, options) => {
            ...
            const validPorts = ports.filter((p) => typeof p === "number" && Number.isInteger(p) && p >= MIN_PORT && p <= MAX_PORT);
        ...
        ipcMain.handle("notification-sound:set-default", async (_event, filename) => {
            if (typeof filename !== "string" || !filename.trim())
                return false;
            const base = path.basename(filename);
            if (!isSafeNotificationSoundFilename(base))
                return false;
            const dir = getDefaultNotificationSoundsDir();
        ...
        ipcMain.handle("notification-sound:get-audio", async (_event, payload) => {

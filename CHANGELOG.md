...
- **CodeQL Java/Kotlin** — Switch from default buildless scanning to an advanced workflow that traces a Gradle debug compile so Kotlin sources and dependencies resolve above CodeQL quality thresholds. Cover Actions/Rust in the same workflow so Default setup can be disabled without losing language coverage.

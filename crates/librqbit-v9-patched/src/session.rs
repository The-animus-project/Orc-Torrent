...
use crate::{
    ...
    mse_stream::{MseReader, MseWriter, PrefixedReader},
...
use orc_mse::{MseStats, MseStatsSnapshot, PeerTrafficMode, TrafficProtection};
use orc_scheduler::SchedulerMode;
...
pub struct Session {
    ...
    request_scheduler: SchedulerMode,
    mse_stats: Arc<MseStats>,
    mse_torrent_lookup: DashMap<[u8; 20], [u8; 20]>,
...
pub struct SessionOptions {
    ...
    /// Do not tolerate partial discovery/listener startup on a selected device.
    pub strict_network_binding: bool,
    ...
    /// Force persisted torrents to restore paused. Embedded hosts use this when
    /// their required network (for example a VPN) is unavailable at startup.
    ...
    /// Disable peer exchange globally. Private torrents always disable PEX.
    pub disable_pex: bool,

    /// Force IPv4 only.
    pub ipv4_only: bool,

    /// ORC peer traffic obfuscation mode. MSE applies to TCP/SOCKS streams only.
    pub peer_traffic_mode: PeerTrafficMode,

    /// ORC request scheduler. Legacy remains the default until the benchmark gate is promoted.
...
fn effective_trackers(
    mut torrent_trackers: Vec<url::Url>,
    session_trackers: &HashSet<url::Url>,
    disable_trackers: bool,
    is_private: bool,
) -> Vec<url::Url> {
    if disable_trackers {
...
pub(crate) struct CheckedIncomingConnection {
    ...
    pub protection: TrafficProtection,
}
...
impl Session {
    ...
    pub fn new_with_opts(
        ...
        async move {
            ...
            if matches!(opts.peer_traffic_mode, PeerTrafficMode::Require) {
                let tcp_requested = opts.connect.as_ref().map(|c| c.enable_tcp).unwrap_or(true)
                    && opts
                        .listen
                        .as_ref()
                        .map(|listen| listen.mode.tcp_enabled())
                        .unwrap_or(true);
                if !tcp_requested {
                    bail!("MSE require mode needs TCP enabled");
                }
                if let Some(listen) = opts.listen.as_mut() {
                    listen.mode = crate::ListenerMode::TcpOnly;
                }
            }
            let peer_id = opts
                .peer_id
            ...
            let mse_stats = Arc::new(MseStats::default());
            let stream_connector = Arc::new(
                StreamConnector::new(StreamConnectorArgs {
                    ...
                    peer_traffic_mode: opts.peer_traffic_mode,
                    mse_stats: mse_stats.clone(),
                })
                .await
            ...
            let lsd = {
                if opts.disable_local_service_discovery {
                    None
                } else if opts.strict_network_binding {
                    Some(
                        ...
                        .await
                        .context("error starting local service discovery")?,
                    )
                } else {
                    LocalServiceDiscovery::new(LocalServiceDiscoveryOptions {
                        cancel_token: token.clone(),
            ...
            let tcp_enabled = opts.connect.as_ref().is_some_and(|value| value.enable_tcp)
                || listen_result
                    .as_ref()
                    .is_some_and(|value| value.tcp_socket.is_some());
            let utp_enabled = listen_result
                .as_ref()
                .is_some_and(|value| value.utp_socket.is_some());
            ...
            let session = Arc::new(Self {
                ...
                utp_enabled,
                peer_traffic_mode: opts.peer_traffic_mode,
                request_scheduler: opts.request_scheduler,
                mse_stats,
                mse_torrent_lookup: DashMap::new(),
    ...
    async fn check_incoming_connection(
        ...
        const BITTORRENT_PREFIX: &[u8; 20] = b"\x13BitTorrent protocol";
        let mut prefix = [0u8; 20];
        tokio::time::timeout(
            orc_mse::DEFAULT_NEGOTIATION_TIMEOUT,
            reader.read_exact(&mut prefix),
        )
        .await
        .context("incoming peer prefix timed out")?
        .context("error reading incoming peer prefix")?;
        let mut protection = TrafficProtection::Plaintext;
        let mut negotiated_info_hash = None;
        let mut read_buf;
        if prefix == *BITTORRENT_PREFIX {
            if matches!(self.peer_traffic_mode, PeerTrafficMode::Require)
                && !matches!(kind, ConnectionKind::Utp)
            {
                self.mse_stats.rejected();
                bail!("plaintext incoming peer rejected by MSE require mode");
            }
            read_buf = ReadBuf::from_bytes(&prefix)?;
        } else if !matches!(kind, ConnectionKind::Utp)
            && !matches!(self.peer_traffic_mode, PeerTrafficMode::Off)
        {
            self.mse_stats.attempted();
            let mut prefixed = PrefixedReader::new(prefix, &mut reader);
            let negotiated = tokio::select! {
                _ = self.cancellation_token.cancelled() => bail!("session cancelled during MSE negotiation"),
                result = orc_mse::negotiate_responder(&mut prefixed, &mut *writer, |hash| {
                    self.mse_torrent_lookup.get(hash).map(|entry| *entry.value())
                }) => result,
            };
            let negotiated = match negotiated {
                Ok(negotiated) => negotiated,
                Err(error) => {
                    self.mse_stats.record_error(&error);
                    return Err(error.into());
                }
            };
            self.mse_stats.succeeded();
            protection = TrafficProtection::MseRc4;
            negotiated_info_hash = Some(negotiated.info_hash);
            read_buf = ReadBuf::from_bytes(&negotiated.initial_payload)?;
            reader = MseReader::boxed(reader, negotiated.decrypt);
            writer = MseWriter::boxed(writer, negotiated.encrypt);
        } else {
            read_buf = ReadBuf::from_bytes(&prefix)?;
        }
        let h = read_buf
        ...
        if negotiated_info_hash.is_some_and(|expected| expected != h.info_hash.0) {
            self.mse_stats.rejected();
            bail!("MSE torrent selection did not match the BitTorrent handshake");
        }
    ...
    async fn add_torrent_internal(
        ...
        let (managed_torrent, metadata) = {
            ...
            self.mse_torrent_lookup
                .insert(orc_mse::request_two_hash(&info_hash.0), info_hash.0);
            (handle, metadata)
        };
            ...
            && let Err(e) = p.store(id, &managed_torrent).await
        {
            ...
            self.mse_torrent_lookup
                .remove(&orc_mse::request_two_hash(&info_hash.0));
            return Err(e);
        }
    ...
    pub async fn delete(&self, id: TorrentIdOrHash, delete_files: bool) -> anyhow::Result<()> {
        ...
        self.connector.clear_torrent_fallbacks(removed.info_hash());
        self.mse_torrent_lookup
            .remove(&orc_mse::request_two_hash(&removed.info_hash().0));
    ...
    fn make_peer_rx(
        ...
        if is_private && !self.disable_trackers {
            // A private torrent may contact only trackers declared by that
            // torrent. Never append session-wide trackers.
            if trackers.len() > 1 {
                warn!(
                    ?info_hash,
        ...
        trackers = effective_trackers(trackers, &self.trackers, self.disable_trackers, is_private);
    ...
    pub fn peer_traffic_mode(&self) -> PeerTrafficMode {
        self.peer_traffic_mode
    }

    pub fn request_scheduler(&self) -> SchedulerMode {
        self.request_scheduler
    }

    pub fn mse_stats(&self) -> MseStatsSnapshot {
        self.mse_stats.snapshot()
    }
...
mod tests {
    ...
    use super::{effective_trackers, torrent_file_from_info_bytes};

    #[test]
    fn private_torrents_never_inherit_session_trackers() {
        let declared: url::Url = "https://private.invalid/announce".parse().unwrap();
        let global: url::Url = "https://global.invalid/announce".parse().unwrap();
        let session_trackers = [global].into_iter().collect();
        let effective = effective_trackers(vec![declared.clone()], &session_trackers, false, true);
        assert_eq!(effective, vec![declared]);
    }

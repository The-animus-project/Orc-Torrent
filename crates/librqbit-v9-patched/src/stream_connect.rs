...
use orc_mse::{MseStats, PeerTrafficMode};
use serde::Serialize;
use tracing::debug;
...
pub(crate) struct StreamConnector {
    ...
    ipv4_only: bool,
    plaintext_fallbacks: dashmap::DashMap<(Id20, SocketAddr), Instant>,
...
impl StreamConnector {
    pub async fn new(config: StreamConnectorArgs) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            plaintext_fallbacks: dashmap::DashMap::new(),
            peer_traffic_mode: config.peer_traffic_mode,
            mse_stats: config.mse_stats,
        })
    }
    ...
    pub(crate) fn peer_traffic_mode(&self) -> PeerTrafficMode {
        self.peer_traffic_mode
    }

    pub(crate) fn mse_stats(&self) -> &Arc<MseStats> {
        &self.mse_stats
    }

    pub(crate) fn plaintext_fallback_cached(&self, info_hash: Id20, addr: SocketAddr) -> bool {
        const TTL: Duration = Duration::from_secs(30 * 60);
        let key = (info_hash, addr);
        let fresh = self
            .plaintext_fallbacks
            .get(&key)
            .is_some_and(|stored| stored.elapsed() < TTL);
        if !fresh {
            self.plaintext_fallbacks.remove(&key);
        }
        fresh
    }

    pub(crate) fn cache_plaintext_fallback(&self, info_hash: Id20, addr: SocketAddr) {
        self.plaintext_fallbacks
            .insert((info_hash, addr), Instant::now());
    }

    pub(crate) fn clear_plaintext_fallback(&self, info_hash: Id20, addr: SocketAddr) {
        self.plaintext_fallbacks.remove(&(info_hash, addr));
    }

    pub(crate) fn clear_torrent_fallbacks(&self, info_hash: Id20) {
        self.plaintext_fallbacks
            .retain(|(stored_hash, _), _| *stored_hash != info_hash);
    }

    pub(crate) async fn connect_tcp_only(
        &self,
        addr: SocketAddr,
        ...
        if !self.enable_tcp {
            return Err(Error::ConnectDisabled);
        }
        let stream = self.tcp_connect(addr).await.map_err(Error::TcpConnect)?;
        let (read, write) = stream.into_split();
        Ok((ConnectionKind::Tcp, Box::new(read), Box::new(write)))
    }

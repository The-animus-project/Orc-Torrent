...
impl ReadBuf {
    ...
    pub(crate) fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() > BUFLEN {
            return Err(Error::ReadBufFull {
                need_additional_bytes: u16::try_from(bytes.len()).unwrap_or(u16::MAX),
            });
        }
    ...
    pub async fn read_handshake(
        ...
        while self.len < HANDSHAKE_LEN {
            let read = with_timeout(
                "reading",
                timeout,
                conn.read(&mut self.buf[self.len..])
...
mod tests {
    ...
    use crate::{
        ...
        type_aliases::BoxAsyncReadVectored,
    ...
    #[tokio::test]
    async fn handshake_preserves_prefixed_and_fragmented_bytes() {
        let handshake = Handshake::new(Id20::new([7u8; 20]), Id20::new([9u8; 20]));
        let mut serialized = [0u8; 68];
        let _ = handshake.serialize_unchecked_len(&mut serialized);
        let (mut writer, reader) = tokio::io::duplex(128);
        writer.write_all(&serialized[20..]).await.unwrap();
        let mut reader: BoxAsyncReadVectored = Box::new(reader.into_vectored_compat());
        let mut read_buf = ReadBuf::from_bytes(&serialized[..20]).unwrap();
        let decoded = read_buf
            .read_handshake(&mut reader, Duration::from_secs(1))
            .await
            .unwrap();

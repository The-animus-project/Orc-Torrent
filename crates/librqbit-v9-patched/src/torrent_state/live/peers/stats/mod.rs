...
impl AggregatePeerStatsAtomic {
    ...
    fn live_protection_counter(&self, l: &LivePeerState) -> &AtomicU32 {
        match l.traffic_protection {
            TrafficProtection::Plaintext => &self.live_plaintext,
            TrafficProtection::MseRc4 => &self.live_mse_rc4,
        }
    }
    ...
    pub(crate) fn inc(&self, state: &PeerState) {
        if let PeerState::Live(l) = state {
            ...
            atomic_inc(self.live_protection_counter(l));
        }
    ...
    pub(crate) fn dec(&self, state: &PeerState) {
        if let PeerState::Live(l) = state {
            ...
            atomic_dec(self.live_protection_counter(l));
        }

...
impl PeerStates {
    pub(crate) fn scheduler_key(addr: SocketAddr) -> u64 {
        let mut hasher = DefaultHasher::new();
        addr.hash(&mut hasher);
        hasher.finish()
    }

    pub(crate) fn endgame_candidate(
        &self,
        recipient: SocketAddr,
    ) -> Option<(SocketAddr, ValidPieceIndex)> {
        let recipient_bitfield = self.with_live(recipient, |live| live.bitfield.clone())?;
        self.states.iter().find_map(|entry| {
            let addr = *entry.key();
            if addr == recipient {
                return None;
            }
            let live = entry.value().get_live()?;
            live.inflight_requests.iter().find_map(|request| {
                recipient_bitfield
                    .get(request.piece_index.get_usize())
                    .is_some_and(|value| *value)
                    .then_some((addr, request.piece_index))
            })
        })
    }

    pub(crate) fn cancel_piece_by_scheduler_key(&self, scheduler_key: u64, piece: ValidPieceIndex) {
        let addr = self.states.iter().find_map(|entry| {
            (Self::scheduler_key(*entry.key()) == scheduler_key).then_some(*entry.key())
        });
        let Some(addr) = addr else { return };
        self.with_live_mut(addr, "endgame_cancel", |live| {
            live.inflight_requests.retain(|request| {
                if request.piece_index != piece {
                    return true;
                }
                let _ = live
                    .tx

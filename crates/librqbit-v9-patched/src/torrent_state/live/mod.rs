...
use orc_scheduler::{
    AssignmentDecision, Block as ScheduledBlock, EndgameConfig, EndgameCoordinator, PeerScheduler,
    SchedulerMode,
};
...
use crate::{
    ...
    limits::Limits,
    peer_connection::{
        InitialAvailability, PeerConnection, PeerConnectionHandler, PeerConnectionOptions,
        WriterRequest,
    },
...
pub struct TorrentStateLive {
    ...
    ratelimits: Limits,
    endgame: Mutex<EndgameCoordinator>,
}
...
impl TorrentStateLive {
    pub(crate) fn new(
        ...
        let state = Arc::new(TorrentStateLive {
            ...
            ratelimit_upload_tx,
            ratelimits,
            endgame: Mutex::new(EndgameCoordinator::new(EndgameConfig::default())),
        });
    ...
    async fn task_manage_incoming_peer(
        ...
        let scheduler_mode = self
            .shared
            .session
            .upgrade()
            .map(|session| session.request_scheduler())
            .unwrap_or_default();
    ...
    fn set_peer_live(
        ...
        protection: orc_mse::TrafficProtection,
    ) {
        self.peers.with_peer_mut(handle, "set_peer_live", |p| {
            p.connecting_to_live(h.peer_id, &self.peers, connection_kind, protection);
        });
    }
...
struct PeerHandler {
    ...
    // ORC-owned adaptive request state. The backend only adapts wire events to this boundary.
    scheduler: Mutex<PeerScheduler>,
    scheduler_started: Instant,
    cancelled_uploads: Mutex<HashSet<ChunkInfo>>,
...
impl PeerConnectionHandler for &'_ PeerHandler {
    ...
    async fn on_received_message(&self, message: Message<'_>) -> anyhow::Result<()> {
        ...
        match message {
            ...
            Message::SuggestPiece(piece) => {
                self.scheduler.lock().suggest_piece(piece);
            ...
            Message::AllowedFast(piece) => {
                self.scheduler.lock().allow_fast(piece);
                self.sync_scheduler_metrics();
                self.refill_request_permits();
                self.unchoke_notify.notify_waiters();
            }
            Message::NotInterested => {
                trace!("received \"not interested\", but we don't process it yet")
            }
            Message::Cancel(request) => self.on_cancel_upload(request),
            ...
            Message::Extended(ExtendedMessage::UtPex(pex)) => {
                if self.state.metadata.info.info().private || self.state.shared.options.disable_pex
                {
                    warn!(
                        id = self.state.shared.id,
                        info_hash = ?self.state.shared.info_hash,
    ...
    fn on_handshake(
        ...
        self.sync_scheduler_metrics();
        self.state
            .set_peer_live(self.addr, handshake, ckind, protection);
        Ok(())
    }
    ...
    fn should_upload_chunk(&self, chunk: &ChunkInfo) -> bool {
        !self.cancelled_uploads.lock().remove(chunk)
    }
    ...
    fn initial_availability(&self) -> InitialAvailability {
        let have = self.state.get_approx_have_bytes();
        if have == 0 {
            InitialAvailability::HaveNone
        } else if have >= self.state.lengths.total_length() {
...
impl PeerHandler {
    ...
    fn scheduled_block(chunk: &ChunkInfo) -> ScheduledBlock {
        ScheduledBlock {
            piece: chunk.piece_index.get(),
            offset: chunk.offset,
            length: chunk.size,
        }
    }

    fn sync_scheduler_metrics(&self) {
        let metrics = self.scheduler.lock().metrics(self.scheduler_now_ms());
        #[allow(clippy::cast_possible_truncation)]
        let choke_ratio_ppm = (metrics.choke_ratio.clamp(0.0, 1.0) * 1_000_000.0) as u32;
        #[allow(clippy::cast_possible_truncation)]
        let reject_ratio_ppm = (metrics.reject_ratio.clamp(0.0, 1.0) * 1_000_000.0) as u32;
        self.counters
            .scheduler_rtt_ms
            .store(metrics.rtt_ms.unwrap_or_default(), Ordering::Relaxed);
        self.counters
            .scheduler_goodput_bps
            .store(metrics.goodput_bytes_per_second, Ordering::Relaxed);
        self.counters
            .scheduler_choke_events
            .store(metrics.choke_events, Ordering::Relaxed);
        self.counters
            .scheduler_choke_ratio_ppm
            .store(choke_ratio_ppm, Ordering::Relaxed);
        self.counters
            .scheduler_reject_events
            .store(metrics.reject_events, Ordering::Relaxed);
        self.counters
            .scheduler_reject_ratio_ppm
            .store(reject_ratio_ppm, Ordering::Relaxed);
        self.counters
            .scheduler_timeout_events
            .store(metrics.timeout_events, Ordering::Relaxed);
        self.counters
            .scheduler_consecutive_timeouts
            .store(metrics.consecutive_timeouts, Ordering::Relaxed);
        self.counters
            .scheduler_available_pieces
            .store(metrics.available_pieces, Ordering::Relaxed);
        self.counters
            .scheduler_total_pieces
            .store(metrics.total_pieces, Ordering::Relaxed);
        self.counters
            .scheduler_outstanding_bytes
            .store(metrics.outstanding_bytes, Ordering::Relaxed);
        self.counters
            .scheduler_outstanding_requests
            .store(metrics.outstanding_requests as u32, Ordering::Relaxed);
        self.counters
            .scheduler_target_requests
            .store(metrics.target_pipeline_requests as u32, Ordering::Relaxed);
        self.counters
            .scheduler_stalled_reassignments
            .store(metrics.stalled_reassignments, Ordering::Relaxed);
    }

    fn refill_request_permits(&self) {
        let wanted = self.scheduler.lock().available_request_slots() as usize;
        let available = self.requests_sem.available_permits();
        if wanted > available {
            self.requests_sem.add_permits(wanted - available);
        } else if available > wanted
            && let Ok(permits) = self
                .requests_sem
                .try_acquire_many((available - wanted).try_into().unwrap_or(u32::MAX))
        {
            permits.forget();
        }
    }

    fn update_scheduler_availability(&self) {
        let available = self
            .state
            .peers
            .with_live(self.addr, |live| {
                live.bitfield
                    ...
                    .map(|bits| bits.count_ones().try_into().unwrap_or(u32::MAX))
                    .unwrap_or_default()
            })
            .unwrap_or_default();
        self.scheduler.lock().set_available_pieces(available);
        self.sync_scheduler_metrics();
    }
    ...
    fn acquire_next_piece(&self) -> crate::Result<Option<ValidPieceIndex>> {
        ...
        let suggested = self
            .scheduler
            .lock()
            .take_suggested_piece()
            .and_then(|piece| self.state.lengths.validate_piece_index(piece));

        let result = self
            .state
            .peers
            .with_live_mut(self.addr, "acquire_next_piece", |live| {
                let choked = self.lock_read("i am choked").i_am_choked;
                ...
                let result = pieces.acquire_piece(AcquireRequest {
                    ...
                    peer_has_piece: |p| {
                        bf.get(p.get() as usize).map(|v| *v) == Some(true)
                            && (!choked || self.scheduler.lock().is_allowed_fast(p.get()))
                    },
                    can_steal: |p| {
    ...
    fn try_endgame_duplicate(&self) -> crate::Result<Option<ValidPieceIndex>> {
        if self.scheduler.lock().mode() != SchedulerMode::Adaptive {
            return Ok(None);
        }
        let remaining_bytes = self
            .state
            .lock_read("endgame_remaining")
            .get_chunks()?
            .get_remaining_bytes();
        let remaining_blocks = remaining_bytes
            .div_ceil(CHUNK_SIZE as u64)
            .try_into()
            .unwrap_or(u32::MAX);
        let Some((primary_peer, piece)) = self.state.peers.endgame_candidate(self.addr) else {
            return Ok(None);
        };
        if !self.scheduler.lock().can_request_piece(piece.get()) {
            return Ok(None);
        }
        let primary_key = PeerStates::scheduler_key(primary_peer);
        let duplicate_key = PeerStates::scheduler_key(self.addr);
        let mut endgame = self.state.endgame.lock();
        endgame.set_remaining_blocks(remaining_blocks);
        if !endgame.is_active() {
            return Ok(None);
        }
        let mut assigned = false;
        for chunk in self.state.lengths.iter_chunk_infos(piece) {
            let block = Self::scheduled_block(&chunk);
            let _ = endgame.assign(primary_key, block);
            assigned |= matches!(
                endgame.assign(duplicate_key, block),
                AssignmentDecision::Duplicate
            );
        }
        Ok(assigned.then_some(piece))
    }

    fn resolve_endgame_piece(&self, piece: ValidPieceIndex) -> crate::Result<()> {
        let winner_key = PeerStates::scheduler_key(self.addr);
        let losers = self
            .state
            .endgame
            .lock()
            .complete_piece(winner_key, piece.get());
        if losers.is_empty() {
            return Ok(());
        }
        ...
        let previous_owner = self
            .state
            .lock_write("claim_endgame_piece")
            .get_pieces_mut()?
            .claim_piece(piece, self.addr);
        if let Some(previous_owner) = previous_owner
            && previous_owner != self.addr
        {
            self.state.peers.on_steal(previous_owner, self.addr, piece);
        }
        for loser in losers {
            self.state.peers.cancel_piece_by_scheduler_key(loser, piece);
        }
        Ok(())
    }
    ...
    fn on_download_request(&self, request: Request) -> anyhow::Result<()> {
        if self.state.torrent().options.disable_upload() {
            return self.reject_download_request(request, "upload disabled");
        }

        let piece_index = match self.state.lengths.validate_piece_index(request.index) {
            Some(p) => p,
            None => {
                return self.reject_download_request(request, "piece index is invalid");
            }
        };
        ...
        let chunk_info = match self.state.lengths.chunk_info_from_received_data(
            ...
            Some(d) => d,
            None => {
                return self.reject_download_request(request, "chunk data is invalid");
            }
        };
        ...
        {
            return self.reject_download_request(request, "chunk is not ready to upload");
        }

        self.cancelled_uploads.lock().remove(&chunk_info);
    ...
    fn on_cancel_upload(&self, request: Request) {
        ...
        let Some(chunk) =
            self.state
                .lengths
                .chunk_info_from_received_data(piece, request.begin, request.length)
        else {
            return;
        };
        self.cancelled_uploads.lock().insert(chunk);
    }

    fn reject_download_request(&self, request: Request, reason: &str) -> anyhow::Result<()> {
        if self
            .scheduler
            .lock()
            .metrics(self.scheduler_now_ms())
            .supports_fast_extension
        {
            trace!(?request, reason, "rejecting request with BEP 6");
            self.tx
                .send(WriterRequest::Message(Message::RejectRequest(request)))
                .map_err(|_| anyhow::anyhow!("peer task died while rejecting request"))?;
            return Ok(());
        }
        anyhow::bail!("received invalid request {request:?}: {reason}")
    }
    ...
    fn on_have(&self, have: u32) {
        ...
        self.on_bitfield_notify.notify_waiters();
        self.update_scheduler_availability();
    }

    fn on_have_all(&self) {
        self.state
            .peers
            .with_live_mut(self.addr, "on_have_all", |live| {
                live.bitfield = make_piece_bitfield(&self.state.lengths);
                if let Some(pieces) = live
                    .bitfield
                    .get_mut(..self.state.lengths.total_pieces() as usize)
                {
                    pieces.fill(true);
                }
            });
        self.scheduler.lock().set_have_all();
        self.sync_scheduler_metrics();
        self.on_bitfield_notify.notify_waiters();
    }
    ...
    fn on_have_none(&self) {
        self.state
            .peers
            .with_live_mut(self.addr, "on_have_none", |live| {
                live.bitfield = make_piece_bitfield(&self.state.lengths);
            });
        self.scheduler.lock().set_have_none();
        self.sync_scheduler_metrics();
        self.on_bitfield_notify.notify_waiters();
    }
    ...
    fn on_reject_request(&self, request: Request) -> anyhow::Result<()> {
        ...
        let removed = self
            .state
            .peers
            .with_live_mut(self.addr, "reject_request", |live| {
                live.inflight_requests.remove(&chunk)
            })
            .unwrap_or_default();
        if removed
            && self
                .scheduler
                .lock()
                .on_reject(Self::scheduled_block(&chunk))
        {
            self.state.endgame.lock().release(
                PeerStates::scheduler_key(self.addr),
                Self::scheduled_block(&chunk),
            );
            let released = self
                .state
                .lock_write("release_rejected_piece")
                .get_pieces_mut()?
                .release_piece_owned_by(self.addr, piece);
            if released {
                self.state.new_pieces_notify.notify_waiters();
            }
        }
    ...
    async fn wait_for_unchoke(&self) {
        self.wait_for_any_notify(&self.unchoke_notify, || {
            !self.lock_read("wait_for_unchoke:i_am_choked").i_am_choked
                || self.scheduler.lock().available_request_slots() > 0
        })
        .await;
    }

    fn reassign_stalled_requests(&self) -> crate::Result<()> {
        let stalled = self.scheduler.lock().take_stalled(self.scheduler_now_ms());
        if stalled.is_empty() {
            return Ok(());
        }
        let mut released_pieces = HashSet::new();
        for block in stalled {
            self.state
                .endgame
                .lock()
                .release(PeerStates::scheduler_key(self.addr), block);
            let Some(piece) = self.state.lengths.validate_piece_index(block.piece) else {
                continue;
            };
            let Some(chunk) =
                self.state
                    .lengths
                    .chunk_info_from_received_data(piece, block.offset, block.length)
            else {
                continue;
            };
            self.state
                .peers
                .with_live_mut(self.addr, "stalled_request", |live| {
                    if live.inflight_requests.remove(&chunk) {
                        let _ = live
                            .tx
            ...
            released_pieces.insert(piece);
        }
        let mut state = self.state.lock_write("release_stalled_pieces");
        let pieces = state.get_pieces_mut()?;
        let mut released = false;
        for piece in released_pieces {
            released |= pieces.release_piece_owned_by(self.addr, piece);
        }
        drop(state);
        if released {
            self.state.new_pieces_notify.notify_waiters();
    ...
    async fn task_peer_chunk_requester(&self) -> crate::Result<()> {
        ...
        loop {
            self.reassign_stalled_requests()?;
            ...
            let acquired = self.acquire_next_piece()?;
            let next = match acquired {
                Some(piece) => Some(piece),
                None => self.try_endgame_duplicate()?,
            };
            let next = match next {
                Some(next) => next,
                None => {
            ...
            for chunk in self.state.lengths.iter_chunk_infos(next) {
                ...
                self.scheduler
                    .lock()
                    .on_request(Self::scheduled_block(&chunk), self.scheduler_now_ms());
                self.sync_scheduler_metrics();
    ...
    fn on_i_am_choked(&self) {
        ...
        self.scheduler
            .lock()
            .set_choked(true, self.scheduler_now_ms());
        self.sync_scheduler_metrics();
        self.refill_request_permits();
    }
    ...
    fn on_i_am_unchoked(&self) {
        ...
        self.scheduler
            .lock()
            .set_choked(false, self.scheduler_now_ms());
        self.sync_scheduler_metrics();
        self.unchoke_notify.notify_waiters();
        self.refill_request_permits();
    }
    ...
    async fn on_received_piece(&self, piece: Piece<ByteBuf<'_>>) -> anyhow::Result<()> {
        ...
        self.scheduler
            .lock()
            .on_block_received(Self::scheduled_block(&chunk_info), self.scheduler_now_ms());
        self.sync_scheduler_metrics();
        self.refill_request_permits();
        self.resolve_endgame_piece(piece_index)?;

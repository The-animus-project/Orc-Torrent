...
use orc_mse::{PeerTrafficMode, TrafficProtection};
...
use crate::{
    mse_stream::{MseReader, MseWriter},
    read_buf::ReadBuf,
...
pub trait PeerConnectionHandler {
    ...
    fn should_upload_chunk(&self, _chunk: &ChunkInfo) -> bool {
        true
    }
...
pub(crate) enum InitialAvailability {
    Bitfield,
    HaveAll,
    HaveNone,
}
...
impl<H: PeerConnectionHandler> PeerConnection<H> {
    ...
    pub async fn manage_peer_incoming(
        ...
        let handshake_supports_fast = incoming.handshake.supports_fast_extension();
        let remote_handshake = incoming.handshake;

        self.handler
            .on_handshake(remote_handshake, incoming.kind, incoming.protection)
            .map_err(Error::Anyhow)?;

        self.manage_peer(ManagePeerArgs {
            handshake_supports_extended,
            handshake_supports_fast,
    ...
    pub async fn manage_peer_outgoing(
        ...
        let now = Instant::now();
        let (mut ckind, mut read, mut write) = with_timeout(
            "connecting",
            connect_timeout,
        ...
        async move {
            ...
            let mut plaintext_fallback_succeeded = false;
            let mode = self.connector.peer_traffic_mode();
            let use_mse = !matches!(ckind, ConnectionKind::Utp)
                && !matches!(mode, PeerTrafficMode::Off)
                && (!matches!(mode, PeerTrafficMode::Prefer)
                    || !self
                        .connector
                        .plaintext_fallback_cached(self.info_hash, self.addr));

            if use_mse {
                self.connector.mse_stats().attempted();
                match orc_mse::negotiate_initiator(
                    &mut *read,
                    &mut *write,
                    self.info_hash.0,
                    &write_buf[..hsz],
                ...
                .await
                {
                    Ok(negotiated) => {
                        ...
                        self.connector.mse_stats().succeeded();
                        self.connector
                            .clear_plaintext_fallback(self.info_hash, self.addr);
                        read = MseReader::boxed(read, negotiated.decrypt);
                        write = MseWriter::boxed(write, negotiated.encrypt);
                    }
                    Err(error) => {
                        self.connector.mse_stats().record_error(&error);
                        if matches!(mode, PeerTrafficMode::Require) {
                            return Err(Error::Anyhow(error.into()));
                        }
                        drop(read);
                        drop(write);
                        (ckind, read, write) = with_timeout(
                            "plaintext fallback connect",
                            connect_timeout,
                            self.connector.connect_tcp_only(self.addr),
                        )
                        .await?;
                        self.connector.mse_stats().fallback();
                        with_timeout(
                            "writing plaintext fallback handshake",
                            rwtimeout,
                            write
                        ...
                        .await?;
                        plaintext_fallback_succeeded = true;
                    }
                }
            } else {
                with_timeout(
            ...
            let handshake_supports_fast = h.supports_fast_extension();
            trace!(
                peer_id=?h.peer_id,
            ...
            if plaintext_fallback_succeeded {
                self.connector
                    .cache_plaintext_fallback(self.info_hash, self.addr);
            }

            self.handler
                .on_handshake(h, ckind, protection)
                .map_err(Error::Anyhow)?;
    ...
    async fn manage_peer(&self, args: ManagePeerArgs) -> Result<()> {
        ...
        let writer = async move {
            ...
            let initial_message =
                handshake_supports_fast.then(|| self.handler.initial_availability());
            if self.handler.should_send_bitfield() || initial_message.is_some() {
                let availability = initial_message.unwrap_or(InitialAvailability::Bitfield);
                let len = match availability {
                    InitialAvailability::Bitfield => self
                        .handler
                        .serialize_bitfield_message_to_buf(&mut *write_buf)
                        .map_err(Error::Anyhow)?,
                    InitialAvailability::HaveAll => {
                        Message::HaveAll.serialize(&mut *write_buf, &Default::default)?
                    }
                    InitialAvailability::HaveNone => {
                        Message::HaveNone.serialize(&mut *write_buf, &Default::default)?
                    }
                };
                with_timeout(
                    "writing bitfield",
                ...
                .await?;
                trace!(?availability, "sent initial availability");
            }
            ...
            loop {
                ...
                let len = match req {
                    ...
                    WriterRequest::ReadChunkRequest(chunk) => {
                        if !self.handler.should_upload_chunk(&chunk) {
                            trace!(?chunk, "skipping cancelled upload request");
                            continue;
                        }
                        #[allow(unused_mut)]

...
impl PieceTracker {
    ...
    /// Release one piece after an explicit BEP 6 rejection or scheduler timeout.
    pub fn release_piece_owned_by(&mut self, peer: PeerHandle, piece: ValidPieceIndex) -> bool {
        if self
            .inflight
            .get(&piece)
            .is_none_or(|inflight| inflight.peer != peer)
        {
            return false;
        }
        self.inflight.remove(&piece);
    ...
    /// Transfer ownership after a bounded endgame duplicate wins its first block.
    pub fn claim_piece(&mut self, piece: ValidPieceIndex, peer: PeerHandle) -> Option<PeerHandle> {
        let inflight = self.inflight.get_mut(&piece)?;
        let previous = inflight.peer;
        inflight.peer = peer;

...
impl Handshake {
    pub fn new(info_hash: Id20, peer_id: Id20) -> Handshake {
        ...
        reserved |= 1 << 20;
        // BEP 6 fast extension (reserved byte 7, bit 2).
        reserved |= 1 << 2;
...
mod tests {
    ...
    const EXTENDED: &[u8] =
        include_bytes!("../../../librqbit-v9-patched/resources/test/extended-handshake.bin");
    ...
    fn test_handshake_serialize() {
        ...
        assert!(se.supports_extended());
        assert!(se.supports_fast_extension());
    ...
    fn test_fast_extension_messages_roundtrip() {
        let request = Request::new(42, 16_384, 16_384);
        let messages = [
        ...
        for message in messages {
            ...
            let len = message
                .serialize(&mut encoded, &Default::default)
                .expect("serialize fast message");
            let (decoded, consumed) =
                Message::deserialize(&encoded[..len], &[]).expect("deserialize fast message");
            assert_eq!(consumed, len);
            match (message, decoded) {
                (Message::SuggestPiece(a), Message::SuggestPiece(b))
                ...
                (Message::RejectRequest(a), Message::RejectRequest(b)) => assert_eq!(a, b),

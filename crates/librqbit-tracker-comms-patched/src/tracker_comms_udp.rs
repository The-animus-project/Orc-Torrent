...
impl UdpTrackerClient {
    pub async fn new(
        ...
        let sock = match bind_ipv4 {
            Some(ip) => tokio::net::UdpSocket::bind((ip, 0u16))
                .await
                .with_context(|| format!("error binding UDP tracker to {ip}"))?,
            None => tokio::net::UdpSocket::bind("0.0.0.0:0")
                .await
...
mod tests {
    ...
    #[tokio::test]
    async fn test_new_respects_bind_ipv4() {
        ...
        let client = UdpTrackerClient::new(CancellationToken::new(), Some(Ipv4Addr::LOCALHOST))
            .await
            .unwrap();
        assert_eq!(

...
async fn h_patch_kill_switch(
    ...
    tokio::spawn(async move {
        match config::load_config().await {
            Ok(mut cfg) => {
                cfg.kill_switch = Some(stored);
                if let Err(e) = config::save_config(&cfg).await {
                    warn!("Failed to persist kill switch config: {e}");
                }
            }
            Err(e) => warn!("Failed to load config for kill switch persist: {e}"),
        }
    });
...
async fn h_add_torrent(
    ...
    opts.output_folder = output_folder.clone();
...
mod tests {
    ...
    fn admin_token_matches_header() {
        let mut h = HeaderMap::new();
        h.insert("x-admin-token", HeaderValue::from_static("secret-token-value-here-32chars"));
    ...
    fn admin_token_wrong_length_rejected() {
        ...
        assert!(!admin_token_authorized(&h, "secret-token-value-here-32chars"));
    }
